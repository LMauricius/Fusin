...
namespace Fusin
{
	...
	*/
	...
	const IOCode KEY_RETURN = Key(CC_RETURN);// Carriage return i.e. 'Enter' key in modern keyboards
	const IOCode KEY_ENTER = Key(CC_ENTER);// 'Enter' key usually produces the Carriage Return character (KEY_RETURN), but I also defined it as KEY_ENTER in case someone from modern times (like me) forgets this
	const IOCode KEY_SHIFT = Key(CC_SHIFT);
	const IOCode KEY_CONTROL = Key(CC_CONTROL);
	const IOCode KEY_ALT = Key(CC_ALT);
